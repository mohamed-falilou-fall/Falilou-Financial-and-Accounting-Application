...
def run():
    ...
    # =========================
    # INITIALISATION / CHARGEMENT DB
    # =========================
    if "hypotheses" not in st.session_state:
        data_db = load_module_data(module="hypotheses", annee="1")

        if data_db:
            st.session_state["hypotheses"] = data_db
        else:
    ...
    if st.button("Imprimer la page"):
        st.subheader("Hypothèses de base – Année 1 (Prêt à imprimer)")

        df_print = pd.DataFrame.from_dict(
            st.session_state["hypotheses"],
            orient="index",
            columns=["Valeur"]
        )
