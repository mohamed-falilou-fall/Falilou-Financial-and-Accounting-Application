...
def run():
    ...
    # Récupérer données depuis les modules précédents
    ...
    # --------------------------
    BFR = bfr_module.get("BFR", 0.0)  # si calculé dans 04_BFR
    ...
    # --------------------------
    RN = cr_module.get("Année 1", {}).get("Résultat net (RN)", 0.0) if cr_module else 0.0
    amortissements = cr_module.get("Année 1", {}).get("Dotations aux amortissements", 0.0) if cr_module else 0.0
    provisions = cr_module.get("Année 1", {}).get("Dotations aux provisions", 0.0) if cr_module else 0.0
    reprises = st.number_input("Reprises", value=0.0, format="%.3f", key="reprises")
    ...
    # --------------------------
    VA = cr_module.get("Année 1", {}).get("Valeur ajoutée (VA)", 0.0) if cr_module else 0.0
